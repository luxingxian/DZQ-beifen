<template>
  <view class="home">
   ...
   <!-- <view
    ...
    <view class="main" id="main">
			<view class="search-box__content" v-if="tabIndex==1 ||tabIndex==2">
			    <view class="search">
			...
			<qui-page-follow v-if="tabIndex==2" ref="follow" @handleClickShare='handleClickShare'></qui-page-follow>
			<qui-topic-list  ref="huati"  v-if="tabIndex==3"></qui-topic-list>
      <qui-content
        class="ivideo"
				 v-if="tabIndex==1"
      ...
      ></qui-content>
      <qui-load-more :status="loadingType"   v-if="tabIndex==1"></qui-load-more>
    </view>
    <!-- #ifdef H5-->
    <view
      class="record"
  ...
  <!--  <view
    ...
    <qui-toast ref="toast"></qui-toast> 
		<!-- 首页返回顶部 -->
    <view class="retureTop" v-if="scrollTop > 300" @click.stop="returnTop">
			<qui-icon
			style="color:white;transform: scale(0.9);" 
			class="auth__header__close" 
			name="icon-fold" size="24">
			</qui-icon>
		</view>
	 </view>
</template>
...
export default {
  ...
  methods: {
    ...
    getCategorieIndex(tagId) {
      for (let i = 0, len = this.categories.length; i < len; i += 1) {
		...
		//  返回顶部
		    returnTop(){
		       　uni.pageScrollTo({ 　　　　　　
		        　 duration: 200,//过渡时间默认为300毫秒，根据需求改动
		　　　　　　scrollTop: 0 
		　　　　});
		    },
    ...
    async toggleTab(dataInfo) {
			// 页面切换，滚动条重置0
		uni.pageScrollTo({
　　　　　　duration: 0,//这个地方过渡时间必须为0，否则运行到手机有问题
　　　　　　scrollTop: 0 
　　　　});
		   this.tabIndex=dataInfo.id;
      // 重置列表
      this.isResetList = true;
      this.pageNum = 1;
      ...
      this.filterList[0].data[dataInfo.index].selected = true;
			...
			if (this.tabIndex == 3 || this.tabIndex==2) {
			  ...
			  // 如果是游客，未登录，点击关注，跳转登录页面，也可以不启用，在后台更改关注对游客的权限，全部为空，就行 
			  // if (this.tabIndex==2) {
    ...
    pullDown() {
      if(this.tabIndex==1){
        ...
        console.log("发现页，上拉加载请求数据")
        this.pageNum += 1;
        this.loadThreads()
      }else if(this.tabIndex==2){
				return;
        console.log("关注页，上拉加载请求数据")
          //this.$refs.follow.more();
      }else{
       console.log("话题页，上拉加载请求数据")
       this.$refs.huati.more()
      }
    },
    // 视频禁止同时播放
